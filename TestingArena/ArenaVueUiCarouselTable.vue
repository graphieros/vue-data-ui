...
const model = createModel([
    ...
    SELECT('caption.style.textAlign', ['left', 'center', 'right'], {
        def: 'left',
    }),
    ...
    SELECT('thead.tr.th.style.textAlign', ['left', 'center', 'right'], {
        def: 'right',
    }),
    ...
    RANGE('tbody.tr.td.alternateOpacity', {
        def: 0.5,
        min: 0,
        max: 1,
        step: 0.01,
    ...
    SELECT('tbody.tr.td.style.textAlign', ['left', 'center', 'right'], {
        def: 'right',
    }),
...
<template>
    <Box comp="VueUiCarouselTable" :dataset="dataset" :config="config">
        ...
        <template #VDUI-local>
            <button @click="toggleAnimVduiLocal">
                TOGGLE ANIMATION (public)
            </button>
        ...
        <template #VDUI-build>
            <button @click="toggleAnimVduiBuild">
                TOGGLE ANIMATION (public)
            </button>
