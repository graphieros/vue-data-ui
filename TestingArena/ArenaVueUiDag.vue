...
const model = createModel([
    ...
    RANGE('style.chart.backgroundPattern.spacingRatio', {
        def: 3,
        min: 1,
        max: 5,
        step: 0.1,
    }),
    RANGE('style.chart.backgroundPattern.dotRadiusRatio', {
        def: 5,
        min: 1,
        max: 12,
        step: 1,
    ...
    RANGE('style.chart.backgroundPattern.opacity', {
        def: 1,
        min: 0,
        max: 1,
        step: 0.1,
    ...
    SELECT('style.chart.layout.rankDirection', ['TB', 'RL', 'BT', 'LR'], {
        def: 'TB',
    }),
    SELECT('style.chart.layout.rankSeparation', [100, 80, 60, 40, 20, 0], {
        def: 60,
    }),
    SELECT('style.chart.layout.nodeSeparation', [100, 75, 50, 25, 0], {
        def: 50,
    }),
    SELECT('style.chart.layout.edgeSeparation', [60, 50, 40, 30, 20, 10, 0], {
        def: 30,
    }),
    ...
    SELECT('style.chart.layout.align', ['UL', 'UR', 'DL', 'DR', ''], {
        def: undefined,
    }),
    SELECT('style.chart.layout.arrowShape', ['undirected', 'normal', 'vee'], {
        def: 'vee',
    }),
    ...
    COLOR('style.chart.nodes.selected.downstreamEdges.stroke', {
        def: '#FF0000',
    }),
    ...
    RANGE('style.chart.midpoints.radius', {
        def: 4,
        min: 2,
        max: 8,
        step: 0.5,
    ...
    RANGE('style.chart.midpoints.strokeWidth', {
        def: 1,
        min: 0.5,
        max: 6,
        step: 0.1,
    ...
    SELECT('style.chart.controls.position', ['top', 'bottom'], {
        def: 'bottom',
    }),
