...
import { VueUiGauge as VueUiGaugeTreeshaken } from "vue-data-ui/vue-ui-gauge";
...
const model = createModel([
    ...
    RANGE("style.chart.animation.speed", { def: 1, min: 0, max: 100 }),
    RANGE("style.chart.animation.acceleration", { def: 1, min: 0, max: 10 }),

    RANGE("style.chart.layout.radiusRatio", { def: 0.8, min: 0.5, max: 1.2, step: 0.01 }),
    RANGE("style.chart.layout.track.size", { def: 1, min: 0.5, max: 2, step: 0.01 }),
    CHECKBOX("style.chart.layout.track.useGradient", { def: true }),
    RANGE("style.chart.layout.track.gradientIntensity", { def: 20, min: 10, max: 30 }),
    COLOR("style.chart.layout.markers.color", { def: "#1A1A1A" }),

    CHECKBOX("style.chart.layout.markers.bold", { def: true }),
    RANGE("style.chart.layout.markers.fontSizeRatio", { def: 1, min: 0, max: 2, step: 0.01 }),
    NUMBER("style.chart.layout.markers.offsetY", { def: 0, min: -100, max: 100 }),
    NUMBER("style.chart.layout.markers.roundingValue", { def: 0, min: 0, max: 12 }),
    CHECKBOX("style.chart.layout.markers.show", { def: true }),
    TEXT("style.chart.layout.markers.prefix", { def: "" }),
    TEXT("style.chart.layout.markers.suffix", { def: "" }),

    CHECKBOX("style.chart.layout.pointer.show", { def: true }),
    SELECT("style.chart.layout.pointer.type", ["rounded", "pointy"], { def: "pointy" }),
    RANGE("style.chart.layout.pointer.size", { def: 1, min: 0.2, max: 1.5, step: 0.01 }),
    COLOR("style.chart.layout.pointer.stroke", { def: "#FFFFFF" }),
    RANGE("style.chart.layout.pointer.strokeWidth", { def: 12, min: 1, max: 48, step: 0.5 }),
    CHECKBOX("style.chart.layout.pointer.useRatingColor", { def: true }),
    COLOR("style.chart.layout.pointer.color", { def: "#CCCCCC" }),
    RANGE("style.chart.layout.pointer.circle.radius", { def: 3, min: 0, max: 48, step: 0.5 }),
    COLOR("style.chart.layout.pointer.circle.stroke", { def: "#1A1A1A" }),
    RANGE("style.chart.layout.pointer.circle.strokeWidth", { def: 2, min: 0, max: 12, step: 0.5 }),
    COLOR("style.chart.layout.pointer.circle.color", { def: "#FFFFFF" }),

    RANGE("style.chart.legend.fontSize", { def: 48, min: 8, max: 100 }),
    TEXT("style.chart.legend.prefix", { def: "$" }),
    TEXT("style.chart.legend.suffix", { def: "€" }),
    NUMBER("style.chart.legend.roundingValue", { def: 1, min: 0, max: 12 }),
    CHECKBOX("style.chart.legend.showPlusSymbol", { def: true }),
    CHECKBOX("style.chart.legend.useRatingColor", { def: true }),
    COLOR("style.chart.legend.color", { def: "#1A1A1A" }),
    CHECKBOX("style.chart.legend.show", { def: true }),

    TEXT("style.chart.title.text", { def: "Lorem ipsum dolor amet" }),
    ...
    TEXT("translations.base", { def: "Base population" }),

    CHECKBOX("style.chart.layout.segmentNames.show", { def: true }),
    CHECKBOX("style.chart.layout.segmentNames.curved", { def: false }),
    RANGE("style.chart.layout.segmentNames.offsetRatio", { def: 1.1, min: 0.5, max: 1.3, step: 0.01 }),
    NUMBER("style.chart.layout.segmentNames.fontSize", { def: 16, min: 8, max: 42 }),
    CHECKBOX("style.chart.layout.segmentNames.useSerieColor", { def: true }),
    COLOR("style.chart.layout.segmentNames.color", { def: "#1A1A1A" }),
    CHECKBOX("style.chart.layout.segmentNames.bold", { def: false }),

    CHECKBOX("style.chart.layout.segmentNames.markers.show", { def: true }),
    CHECKBOX("style.chart.layout.segmentNames.markers.useSerieColor", { def: true }),
    COLOR("style.chart.layout.segmentNames.markers.color", { def: "#1A1A1A" }),

    CHECKBOX("style.chart.layout.segmentSeparators.show", { def: false }),
    NUMBER("style.chart.layout.segmentSeparators.offsetOut", { def: 0, min: 0, max: 100 }),
    NUMBER("style.chart.layout.segmentSeparators.offsetIn", { def: 0, min: 0, max: 100 }),
    COLOR("style.chart.layout.segmentSeparators.stroke", { def: "#1A1A1A" }),
    NUMBER("style.chart.layout.segmentSeparators.strokeWidth", { def: 2, min: 0, max: 12 }),

    CHECKBOX("style.chart.layout.indicatorArc.show", { def: true }),
    COLOR("style.chart.layout.indicatorArc.fill", { def: "#E1E5E890" }),
    NUMBER("style.chart.layout.indicatorArc.radius", { def: 123, min: 0, max: 200 }),
...
<template>
    ...
    <Box comp="VueUiGauge" :dataset="isPropsToggled ? alternateDataset : dataset" :config="config">
        ...
        <template #build-treesh>
            <VueUiGaugeTreeshaken :dataset="isPropsToggled ? alternateDataset : dataset" :config="isPropsToggled ? alternateConfig : config" :key="`build_${step}`">
                <template #pattern="{ seriesIndex, patternId }">
                    <pattern v-if="seriesIndex === 0" :id="patternId" width="70" height="8" patternTransform="scale(2)" patternUnits="userSpaceOnUse"><rect width="100%" height="100%" fill="#2b2b3100"/><path fill="none" stroke="#1A1A1A" d="M-.02 22c8.373 0 11.938-4.695 16.32-9.662C20.785 7.258 25.728 2 35 2s14.215 5.258 18.7 10.338C58.082 17.305 61.647 22 70.02 22M-.02 14.002C8.353 14 11.918 9.306 16.3 4.339 20.785-.742 25.728-6 35-6S49.215-.742 53.7 4.339c4.382 4.967 7.947 9.661 16.32 9.664M70 6.004c-8.373-.001-11.918-4.698-16.3-9.665C49.215-8.742 44.272-14 35-14S20.785-8.742 16.3-3.661C11.918 1.306 8.353 6-.02 6.002"/></pattern>
