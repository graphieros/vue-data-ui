...
const model = createModel([
    ...
    TEXT('style.image.src', {
        def: 'https://vue-data-ui.graphieros.com/logo.png',
    }),
    TEXT('style.image.alt', { def: 'Rating image' }),
    NUMBER('style.image.inactiveOpacity', {
        def: 0.3,
        min: 0,
        max: 1,
        step: 0.1,
    }),
    SELECT('style.title.textAlign', ['left', 'center', 'right'], {
        def: 'center',
    }),
    ...
    SELECT('style.rating.position', ['top', 'right', 'bottom', 'left'], {
        def: 'bottom',
    }),
    ...
    <Box comp="VueUiRating" :dataset="dataset" :config="config">
        ...
        <template #local>
            ...
            >
                ...
                <template #layer-under="{ value, size, focusedValue }">
                    <svg viewBox="0 0 10 10" :style="{ overflow: 'visible' }">
                        <path
                            v-if="value === 1"
                            :stroke="
                                focusedValue === value ? '#6A6A6A' : '#CCCCCC'
                            "
                ...
                <template
                    #layer-above="{ value, size, hoveredValue, focusedValue }"
                >
                    <svg viewBox="0 0 10 10" :style="{ overflow: 'visible' }">
                        <path
                            v-if="value === 1"
                            ...
                            :style="{
                                opacity:
                                    value <= r || value < hoveredValue ? 1 : 0,
                            }"
                        />
                        <path
                            v-if="value === 2"
