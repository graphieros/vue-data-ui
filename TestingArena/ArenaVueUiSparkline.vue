...
const model = createModel([
    ...
    NUMBER('style.verticalIndicator.strokeWidth', {
        def: 1.5,
        min: 0,
        max: 6,
        step: 0.5,
    ...
    NUMBER('style.verticalIndicator.strokeDasharray', {
        def: 3,
        min: 0,
        max: 48,
    }),
    ...
    SELECT('style.dataLabel.valueType', ['latest', 'sum', 'average'], {
        def: 'latest',
    }),
    ...
    <Box comp="VueUiSparkline" :dataset="dataset" :config="config">
        ...
        <template #local>
            ...
            >
                ...
                <template
                    #before="{ selected, latest, sum, average, median, trend }"
                >
