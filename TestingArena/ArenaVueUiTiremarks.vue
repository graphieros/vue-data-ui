...
const model = createModel([
    ...
    SELECT('style.chart.layout.display', ['horizontal', 'vertical'], {
        def: 'horizontal',
    }),
    ...
    RANGE('style.chart.layout.ticks.gradient.shiftHueIntensity', {
        def: 100,
        min: 0,
        max: 100,
    }),
    ...
    SELECT('style.chart.percentage.verticalPosition', ['top', 'bottom'], {
        def: 'bottom',
    }),
    SELECT(
        'style.chart.percentage.horizontalPosition',
        ['left', 'right', 'top'],
        { def: 'left' },
    ),
