...
for (let i = 0; i < 50; i += 1) {
    ...
    crazyDs.push(
        Math.random() + (Math.random() > 0.5 ? Math.random() * 10 : -10),
    );
    crazyDs2.push(
        Math.random() + (Math.random() > 0.5 ? Math.random() * -10 : -10),
    );
...
const model = createModel([
    ...
    NUMBER('style.chart.grid.y.axisLabels.fontSizeRatio', {
        def: 0.7,
        min: 0.1,
        max: 2,
        step: 0.1,
    ...
    NUMBER('style.chart.grid.y.axisLabels.offsetX', {
        def: 0,
        min: -100,
        max: 100,
    }),
    ...
    NUMBER('style.chart.grid.y.verticalLines.hideUnderXLength', {
        def: 0,
        min: 5,
        max: 40,
    }),
    SELECT('style.chart.grid.y.verticalLines.position', ['start', 'middle'], {
        def: 'middle',
    }),

    CHECKBOX('style.chart.grid.x.timeLabels.show', { def: true }),
    NUMBER('style.chart.grid.x.timeLabels.fontSizeRatio', {
        def: 0.8,
        min: 0.1,
        max: 2,
        step: 0.1,
    }),
    NUMBER('style.chart.grid.x.timeLabels.rotation', {
        def: 0,
        min: -360,
        max: 360,
    }),
    ...
    NUMBER('style.chart.grid.x.horizontalLines.opacity', {
        def: 20,
        min: 0,
        max: 100,
    }),
    ...
    NUMBER('style.chart.line.plots.radiusRatio', {
        def: 1,
        min: 0.1,
        max: 3,
        step: 0.1,
    ...
    NUMBER('style.chart.dataLabels.fontSizeRatio', {
        def: 1,
        min: 0.1,
        max: 2,
        step: 0.1,
    ...
    NUMBER('style.chart.paddingProportions.top', {
        def: 0.1,
        min: 0,
        max: 3,
        step: 0.01,
    }),
    NUMBER('style.chart.paddingProportions.right', {
        def: 0.05,
        min: 0,
        max: 3,
        step: 0.01,
    }),
    NUMBER('style.chart.paddingProportions.left', {
        def: 0.1,
        min: 0,
        max: 3,
        step: 0.01,
    }),
    NUMBER('style.chart.paddingProportions.bottom', {
        def: 0.1,
        min: 0,
        max: 3,
        step: 0.01,
