...
function matchRoute(pathName) {
    if (pathName === '/api/items')
        return { routeName: 'itemsRoot', identifier: null };

    const match = pathName.match(/^\/api\/items\/([^/]+)$/);
    if (match)
        return {
            routeName: 'itemsByIdentifier',
...
const server = http.createServer(async (request, response) => {
    try {
        ...
        if (routeName === 'itemsRoot' && request.method === 'GET') {
            writeJsonResponse(request, response, 200, {
        ...
        if (routeName === 'itemsByIdentifier' && request.method === 'GET') {
            const item = database.items.find(
                (entry) => String(entry.id) === String(identifier),
            );
            if (!item) {
                writeJsonResponse(request, response, 404, {
        ...
        if (routeName === 'itemsRoot' && request.method === 'POST') {
            ...
            if (body === undefined) {
                writeJsonResponse(request, response, 400, {
        ...
        writeJsonResponse(request, response, 405, {
        ...
        if (String(error?.message) === 'Request body too large') {
            writeJsonResponse(request, response, 413, {
