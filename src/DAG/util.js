...
function simplify(graph) {
    ...
    graph.edges().forEach((edge) => {
        const simpleLabel = simplified.edge(edge.v, edge.w) || {
            weight: 0,
            minlen: 1,
        };
...
function asNonCompoundGraph(graph) {
    const simplified = new Graph({ multigraph: graph.isMultigraph() }).setGraph(
        graph.graph(),
    );
...
function successorWeights(graph) {
    const weightMap = graph.nodes().map((nodeId) => {
        ...
        graph.outEdges(nodeId).forEach((edge) => {
            successors[edge.w] =
                (successors[edge.w] || 0) + graph.edge(edge).weight;
        });
        return successors;
    });
...
function predecessorWeights(graph) {
    const weightMap = graph.nodes().map((nodeId) => {
        ...
        graph.inEdges(nodeId).forEach((edge) => {
            predecessors[edge.v] =
                (predecessors[edge.v] || 0) + graph.edge(edge).weight;
        });
        return predecessors;
    });
