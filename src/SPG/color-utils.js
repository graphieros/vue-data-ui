...
export function colorToRgba(inputColor, referenceElement = null) {
    ...
    const resolvedColor = resolveCurrentColorIfNeeded(
        trimmed,
        referenceElement,
    );
    ...
    if (after === before && !isColorLikelyBlack(resolvedColor)) {
        throw new Error(
            `colorToRgba: unsupported or invalid color "${inputColor}".`,
        );
    }
 ...
 */
export function interpolateRgbaColor(from, to, t, colorSpace) {
