...
function expectRgbaClose(actual, expected, alphaEpsilon = 1 / 255) {
    ...
    expect(Math.abs(actual.alpha - expected.alpha)).toBeLessThanOrEqual(
        alphaEpsilon,
    );
}
...
function pixelFromFillStyle(fillStyle) {
    ...
    if (v === 'white' || v === '#ffffff' || v === '#fff')
        return [255, 255, 255, 255];
    if (v === 'red' || v === '#ff0000' || v === '#f00') return [255, 0, 0, 255];
    if (v === 'blue' || v === '#0000ff' || v === '#00f')
        return [0, 0, 255, 255];
    ...
    Object.defineProperty(HTMLCanvasElement.prototype, 'getContext', {
        ...
        value: function () {
            ...
            return {
                ...
                getImageData() {
                    const [r, g, b, a] =
                        pixelFromFillStyle(lastPaintedFillStyle);
                ...
                set fillStyle(next) {
                    currentFillStyle = normalizeFillStyleOrKeep(
                        currentFillStyle,
                        String(next),
                    );
                },
...
describe('color-utils', () => {
    ...
    describe('colorToRgba', () => {
        it('throws on empty input', () => {
            expect(() => colorToRgba('')).toThrow(
                /inputColor must be a non-empty string/i,
            );
            expect(() => colorToRgba('   ')).toThrow(
                /inputColor must be a non-empty string/i,
            );
        ...
        it('parses "transparent" as alpha 0', () => {
            expectRgbaClose(colorToRgba('transparent'), {
                red: 0,
                green: 0,
                blue: 0,
                alpha: 0,
        ...
        it('parses hex colors (#rgb and #rrggbb)', () => {
            expectRgbaClose(colorToRgba('#f00'), {
                red: 255,
                green: 0,
                blue: 0,
                alpha: 1,
            });
            expectRgbaClose(colorToRgba('#00ff88'), {
                red: 0,
                green: 255,
                blue: 136,
                alpha: 1,
        ...
        it('parses rgb() and rgba()', () => {
            expectRgbaClose(colorToRgba('rgb(0, 128, 255)'), {
                red: 0,
                green: 128,
                blue: 255,
                alpha: 1,
        ...
        it('parses named colors', () => {
            expectRgbaClose(colorToRgba('red'), {
                red: 255,
                green: 0,
                blue: 0,
                alpha: 1,
            });
            expectRgbaClose(colorToRgba('black'), {
                red: 0,
                green: 0,
                blue: 0,
                alpha: 1,
            });
            expectRgbaClose(colorToRgba('white'), {
                red: 255,
                green: 255,
                blue: 255,
                alpha: 1,
            });
            expectRgbaClose(colorToRgba('blue'), {
                red: 0,
                green: 0,
                blue: 255,
                alpha: 1,
        ...
        it('throws on invalid/unsupported colors (except black-like strings)', () => {
            expect(() => colorToRgba('not-a-color')).toThrow(
                /unsupported or invalid color/i,
            );
            expect(() => colorToRgba('rgb(999, 0, 0)')).toThrow(
                /unsupported or invalid color/i,
            );
        });
        ...
        it('keeps "currentColor" unresolved without a reference element (throws in this environment)', () => {
            expect(() => colorToRgba('currentColor')).toThrow(
                /unsupported or invalid color/i,
            );
        });
    });
    ...
    describe('colorsToRgba', () => {
        it('throws on empty array', () => {
            expect(() => colorsToRgba([])).toThrow(
                /colors must be a non-empty array/i,
            );
            expect(() => colorsToRgba(null)).toThrow(
                /colors must be a non-empty array/i,
            );
        });
    ...
    describe('rgbaToCssRgba', () => {
        ...
        it('rounds alpha to the requested decimals', () => {
            const css = rgbaToCssRgba(
                { red: 1, green: 2, blue: 3, alpha: 0.123456 },
                3,
            );
    ...
    describe('rgbaToCss', () => {
        it('returns hex when alpha >= 1', () => {
            expect(rgbaToCss({ red: 255, green: 0, blue: 136, alpha: 1 })).toBe(
                '#ff0088',
            );
            expect(rgbaToCss({ red: 1, green: 2, blue: 3, alpha: 5 })).toBe(
                '#010203',
            );
        });
        ...
        it('returns rgba() when alpha < 1', () => {
            expect(
                rgbaToCss({ red: 255, green: 0, blue: 0, alpha: 0.5 }, 2),
...
describe('srgbChannelToLinear', () => {
    ...
    it('converts a known mid value close to expected (128)', () => {
        ...
        expect(Math.abs(v - 0.214)).toBeLessThanOrEqual(0.002);
    });
});
...
describe('interpolateCssColors', () => {
    ...
    it('returns rgba() when any endpoint has alpha < 1', () => {
        const out = interpolateCssColors(
            'rgba(0, 0, 0, 0.0)',
            'rgba(255, 0, 0, 1)',
            0.5,
            'srgb',
