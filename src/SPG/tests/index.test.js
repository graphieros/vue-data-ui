    ...
    return {
        colorsToRgba: (colors) => {
            ...
            for (let i = 0; i < colors.length; i += 1)
                out[i] = i === colors.length - 1 ? Blue : Red;
            return out;
        },
    };
});
...
describe('SvgPathGradient (only public function behavior)', () => {
    ...
    it('throws when pathData is empty or not a string', async () => {
        ...
        expect(() => SvgPathGradient('', ['#f00'])).toThrow(
            /pathData must be a non-empty string/i,
        );
        expect(() => SvgPathGradient(null, ['#f00'])).toThrow(
            /pathData must be a non-empty string/i,
        );
    });
    ...
    it('throws when colors is empty or not an array', async () => {
        ...
        expect(() => SvgPathGradient('M 0 0 L 10 0', [])).toThrow(
            /colors must be a non-empty array/i,
        );
        expect(() => SvgPathGradient('M 0 0 L 10 0', null)).toThrow(
            /colors must be a non-empty array/i,
        );
    });
