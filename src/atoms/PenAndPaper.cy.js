...
describe('<PenAndPaper />', () => {
    ...
    it('draws lines', () => {
        ...
        cy.get('[data-cy="pen-and-paper-thickness"]')
            .invoke('val', 12)
            .trigger('input');
        ...
        cy.get('.vue-ui-pen-and-paper-path')
            .eq(0)
            .should('have.attr', 'stroke-width', 1);
        cy.get('.vue-ui-pen-and-paper-path')
            .eq(1)
            .should('have.attr', 'stroke-width', 12);
        ...
        cy.get('[data-cy="color-picker-icon"]')
            .invoke('css', 'background-color')
            .should('equal', 'rgb(255, 87, 51)');
        cy.get('[data-cy="pen-and-paper"]')
            .as('svg')
            .trigger('mousedown', { clientX: 200, clientY: 100, force: true })
            .trigger('mousemove', { clientX: 300, clientY: 200, force: true })
    ...
    it('renders text', () => {
        ...
        cy.get('[data-cy="pen-and-paper-font-size"]')
            .invoke('val', 24)
            .trigger('input');

        cy.log('Click on the canvas to start text editing');
        cy.get('[data-cy="pen-and-paper"]').trigger('mousedown', {
            clientX: 150,
            clientY: 180,
            force: true,
        });
        ...
        cy.window().then((win) => {
            ['h', 'e', 'l', 'l', 'o', 'Enter', 'ع', 'ر', 'ب', 'ي'].forEach(
                (char) => {
                    win.dispatchEvent(
        ...
        cy.get('[data-cy="pen-and-paper"] g text')
            .should('exist')
            .and('be.visible');

        cy.log('Expect multiple tspan for multiple lines');
        cy.get('[data-cy="pen-and-paper"] g text tspan').should(
            'have.length.at.least',
            2,
        );

        cy.log('Text should include both English and Arabic');
        cy.get('[data-cy="pen-and-paper"] g text').should(
            'contain.text',
            'hello',
        );
        cy.get('[data-cy="pen-and-paper"] g text').should(
