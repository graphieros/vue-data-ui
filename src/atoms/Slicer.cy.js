...
describe('<Slicer />', () => {
    ...
    it('uses range handles and updates bounds', () => {
        mountSlicer().then((cmp) => {
            ...
            cy.get('[data-cy="slicer-label-left"]')
                ...
                .should('exist')
                .and('be.visible')
                .and('contain', '____ 1 ____');
            cy.get('[data-cy="slicer-label-right"]')
                .as('right')
                .should('exist')
                .and('be.visible')
                .and('contain', '____ 7 ____');
        });
    });
    ...
    it('drags selection and updates bounds from the minimap', () => {
        mountSlicer().then((cmp) => {
            ...
            cy.get('[data-cy="slicer-label-left"]')
                ...
                .should('exist')
                .and('be.visible')
                .and('contain', '____ 12 ____');
            cy.get('[data-cy="slicer-label-right"]')
                .as('right')
                .should('exist')
                .and('be.visible')
                .and('contain', '____ 19 ____');
        });
    });
    ...
    it('merges the two labels in a single tooltip when tooltips collide', () => {
        mountSlicer().then((cmp) => {
            ...
            cy.get('[data-cy="slicer-label-merged"]')
                .should('be.visible')
                .and('contain', '____ 15 ____ - ____ 16 ____');
            cy.get('[data-cy="slicer-label-left"]').should('not.be.visible');
            cy.get('[data-cy="slicer-label-right"]').should('not.be.visible');
        });
    });
