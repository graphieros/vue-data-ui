...
describe('<VueUi3dBar />', () => {
    it('renders with simple dataset', () => {
        ...
        cy.mount(VueUi3dBar, {
            props: {
                dataset: components.find((c) => c.name === 'VueUi3dBar')
                    .dataset,
            },
        }).then(() => {
            testCommonFeatures({ userOptions: true });
            cy.log('Data label');
            cy.get('[data-cy="vue-ui-3d-bar-simple-datalabel"]')
                .should('exist')
                .and('contain', '100%');
        });
    });
    ...
    it('renders with breakdown dataset', () => {
        ...
        cy.mount(VueUi3dBar, {
            ...
            cy.get('.vue-ui-3d-bar-stack')
                .should('exist')
                .and('have.length', 6);
            dataset.series.forEach((ds) => {
                cy.get(`[data-cy="bar-3d-value-${ds.value}"]`)
                    .should('exist')
                    .and('be.visible');
            });
        });
            ...
            cy.get('.vue-ui-3d-bar-stack')
                ...
                .then(() => {
                    cy.wrap(wrapper.vm.$nextTick()).then(() => {
                        ...
                        expect(
                            wrapper.emitted('selectDatapoint').length,
                        ).to.equal(2);
                    });
                });
        });
