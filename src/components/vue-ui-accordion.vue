...
<template>
    <div class="vue-data-ui-component">
        ...
        >
            <summary
                :class="{ 'vue-ui-accordion-headless': hideDetails }"
                :style="{ cursor: isCursorPointer ? 'pointer' : 'default' }"
            >
                <div
                    class="vue-ui-accordion-head"
                    :style="`background:${FINAL_CONFIG.head.backgroundColor};padding:${FINAL_CONFIG.head.padding}; ${hideDetails ? 'height: 0px !important; padding: 0 !important;' : ''}`"
                >
                    <div class="vue-ui-accordion-arrow" v-if="!hideDetails">
                        <slot
                            name="arrow"
                            v-if="FINAL_CONFIG.head.useArrowSlot"
                            v-bind="{
                                backgroundColor:
                                    FINAL_CONFIG.head.backgroundColor,
                                color: FINAL_CONFIG.head.color,
                                iconColor: FINAL_CONFIG.head.iconColor,
                                isOpen,
                            }"
                        />
                        <BaseIcon
                            :name="FINAL_CONFIG.head.icon"
                            v-else
                            :stroke="FINAL_CONFIG.head.iconColor"
                            :size="FINAL_CONFIG.head.iconSize"
                        />
                    </div>
                    <slot
                        name="title"
                        v-bind="{ color: FINAL_CONFIG.head.color, isOpen }"
                    />
                </div>
            </summary>
        </details>
        <div
            ...
            :style="`background:${FINAL_CONFIG.body.backgroundColor};color:${FINAL_CONFIG.body.color}`"
        >
            <slot
                name="content"
                v-bind="{
                    backgroundColor: FINAL_CONFIG.body.backgroundColor,
                    color: FINAL_CONFIG.body.color,
                    isOpen,
                }"
            />
        </div>
