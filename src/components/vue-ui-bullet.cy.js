...
describe('<VueUiBullet />', () => {
    it('renders', () => {
        cy.mount(VueUiBullet, {
            ...
            cy.log('Legend items');
            cy.get('.vue-data-ui-legend-item')
                .as('legend')
            ...
            cy.get('[data-cy="vue-ui-bullet-segment"]')
                .should('exist')
                .and('be.visible')
                .and('have.length', 3);

            cy.log('Bullet target');
            cy.get('[data-cy="vue-ui-bullet-target-top"]')
                .should('exist')
                .and('be.visible');

            cy.log('Value label');
            cy.get('[data-cy="vue-ui-bullet-value-label"]')
                .should('exist')
                .and('be.visible')
                .and('contain', dataset.value);

            cy.log('Value bar');
            cy.get('[data-cy="vue-ui-bullet-value-bar"]')
                .should('exist')
                .and('be.visible');

            cy.log('Ticks');
            cy.get('[data-cy="vue-ui-bullet-tick-label"]')
                .as('tickLabels')
                .should('exist')
                .and('be.visible')
                .and('have.length', 11);
            ...
            cy.get('[data-cy="vue-ui-bullet-tick-marker"]')
                .should('exist')
                .and('have.length', 11);
        });
    });
