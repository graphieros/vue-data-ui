...
const rootTotals = dataset
    .map((d) => {
        return d.branches.map((b) => b.value).reduce((a, b) => a + b, 0);
    })
    .sort((a, b) => b - a);

const branchesTotals = dataset
    .flatMap((d) => {
...
describe('<VueUiChestnut />', () => {
            ...
            for (let i = 0; i < branchesTotals.length; i += 1) {
                cy.get(`[data-cy="chestnut-branch-${i}"]`).then(($branch) => {
                    ...
                    if (i === branchesTotals.length - 1) {
                        cy.wrap($branch).wait(50).click({ force: true });
                    }
                });

                cy.get(`[data-cy="chestnut-nut-${i}"]`).should('exist');

                cy.get(`[data-cy="chestnut-trap-${i}"]`).then(($trap) => {
                    cy.wrap($trap).wait(50).click({ force: true });
    ...
    it('emits', () => {
        cy.mount(VueUiChestnut, {
            ...
            cy.log('@selectRoot');
            cy.get('[data-cy="chestnut-root-0"]')
                .click({ force: true })
                .then(() => {
                    ...
                    expect(
                        wrapper.emitted('selectRoot')[0][0].rootIndex,
                    ).to.equal(0);
                    expect(wrapper.emitted('selectRoot')[0][0].type).to.equal(
                        'root',
                    );
                });

            cy.log('@selectBranch');
            cy.get('[data-cy="chestnut-branch"]')
                .first()
                .click({ force: true })
                .then(() => {
                    ...
                    expect(
                        wrapper.emitted('selectBranch')[0][0].rootIndex,
                    ).to.equal(0);
                    expect(wrapper.emitted('selectBranch')[0][0].type).to.equal(
                        'branch',
                    );
                });

            cy.log('@selectNut');
            cy.get('[data-cy="chestnut-trap-0"]')
                .click({ force: true })
                .then(() => {
                    expect(wrapper.emitted('selectNut')).to.exist;
                    expect(wrapper.emitted('selectNut')[0][0]).to.have.length(
                        3,
                    );
                    ...
                    expect(wrapper.emitted('selectNut')[0][0][0].type).to.equal(
                        'nut',
                    );
                    expect(
                        wrapper.emitted('selectNut')[0][0][0].rootIndex,
                    ).to.equal(0);
                });
        });
    });
