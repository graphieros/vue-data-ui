    ...
    it('renders', () => {
        ...
        cy.mount(VueUiDonutEvolution, {
            ...
            cy.get('[data-cy="axis-x"]')
                .should('exist')
                .and('have.css', 'opacity', '1');
            cy.get('[data-cy="vertical-separator"]')
                .should('exist')
                .and('have.css', 'opacity', '1')
                .and(
                    'have.length',
                    Math.max(...dataset.map((d) => d.values.length)),
                );
            ...
            cy.get('[data-cy="axis-x-label"]')
                ...
                .and(
                    'have.length',
                    config.style.chart.layout.grid.xAxis.dataLabels.values
                        .length,
                );
            cy.get('@xLabels')
                .first()
                .contains(
                    config.style.chart.layout.grid.xAxis.dataLabels.values[0],
                );
            cy.get('@xLabels')
                .last()
                .contains(
                    config.style.chart.layout.grid.xAxis.dataLabels.values.at(
                        -1,
                    ),
                );
            ...
            cy.get('[data-cy="draggable-dialog-close"]')
                .should('be.visible')
                .click({ force: true });
