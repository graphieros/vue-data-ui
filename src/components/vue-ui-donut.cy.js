...
describe('<VueUiDonut />', () => {
    ...
    function testDataLabels(variation) {
        ...
        cy.get(`[data-cy="${variation}-label-marker"]`)
            .should('exist')
            .and('be.visible')
            .and('have.length', dataset.length);
        cy.get(`[data-cy="${variation}-label-value"]`)
            .as('values')
            .should('exist')
            .and('be.visible')
            .and('have.length', dataset.length);
        cy.get('@values').each((val, i) => {
            cy.wrap(val)
                .should(
                    'contain',
                    dataset[i].values.reduce((a, b) => a + b, 0),
                )
                .and('contain', '%');
        });
        cy.get(`[data-cy="${variation}-label-name"]`)
            .as('names')
            .should('exist')
            .and('be.visible')
            .and('have.length', dataset.length);
    ...
    function testPlotComments() {
        cy.get('[data-cy="slot-plot-comment"]')
            .as('comments')
            .should('exist')
            .and('be.visible')
            .and('have.length', dataset.length);
    ...
    it('renders donut default variation', () => {
        cy.mount(VueUiDonut, {
            ...
            cy.get('[data-cy="donut-arc"]')
                .should('exist')
                .and('be.visible')
                .and('have.length', dataset.length);
            cy.get('[data-cy="donut-shadow"]')
                .should('exist')
                .and('be.visible');

            cy.log('hollow');
            cy.get('[data-cy="hollow-total-name"]')
                .should('exist')
                .and('be.visible')
                .contains('Total');
            cy.get('[data-cy="hollow-total-value"]')
                .should('exist')
                .and('be.visible')
                .contains(total.toFixed(0));
            cy.get('[data-cy="hollow-average-name"]')
                .should('exist')
                .and('be.visible')
                .contains('Average');
            cy.get('[data-cy="hollow-average-value"]')
                .should('exist')
                .and('be.visible')
                .contains(average.toFixed(0));
    ...
    it('renders polar variation', () => {
        cy.mount(VueUiDonut, {
            ...
            cy.get('[data-cy="polar-datapoint"]')
                .should('exist')
                .and('be.visible')
                .and('have.length', dataset.length);
            cy.get('[data-cy="polar-shadow"]')
                .should('exist')
                .and('be.visible')
                .and('have.length', dataset.length);
