    ...
    if (FINAL_CONFIG.value.responsive) {
        const handleResize = throttle(() => {
            const { width, height } = useResponsive({
                ...
                padding: FINAL_CONFIG.value.autoSize
                    ? undefined
                    : padding.value,
            });
...
watch(
    ...
    async (isLoadingNow) => {
        ...
        const startAnimationIsEnabled = FINAL_CONFIG.value.startAnimation?.show;
...
const immutableSet = computed(() => {
    return FINAL_DATASET.value
        .sort((a, b) => {
            const sumA = Array.isArray(a.values)
                ? a.values.reduce((x, y) => x + y, 0)
                : (a.value ?? 0);
            const sumB = Array.isArray(b.values)
...
function segregate(index) {
    ...
    if (segregated.value.includes(index)) {
        ...
        if (
            FINAL_CONFIG.value.serieToggleAnimation.show &&
            FINAL_CONFIG.value.type === 'classic'
        ) {
            doAnimUp();
        } else {
            setFinalUpState();
...
const donutSet = computed(() => {
    if (isFirstLoad.value && !loading.value) {
        ...
        return mutableSet.value
            ...
            .filter(
                (_, seriesIndex) => !segregated.value.includes(seriesIndex),
            );
    }
});
    ...
    return FINAL_DATASET.value
        .map((serie, i) => {
            ...
            const proportion =
                value /
                FINAL_DATASET.value
                    .map((m) => (m.values || []).reduce((a, b) => a + b, 0))
                    .reduce((a, b) => a + b, 0);
            return {
                name: serie.name,
                color:
...
const hasData = computed(() => {
    return (
        Math.abs(
            donutSet.value.map((d) => d.value).reduce((a, b) => a + b, 0),
        ) > 0
    );
});
...
const noGhostDonut = computed(() =>
    currentDonut.value.filter((el) => !el.ghost),
);
...
function isSmallArc(arc, seriesIndex) {
    ...
    return percentage > minimumVisible && percentage <= threshold;
}
...
function legendPercentage(legend) {
    return isAnimating.value
        ? legend.proportion * 100
        : (legend.value / total.value) * 100;
}
...
function useTooltip({
    ...
    activeTooltipIndex.value = relativeIndex;
...
const dataTable = computed(() => {
    ...
    const body = table.value.head.map((h, i) => {
        return [
            ...
            isNaN(table.value.body[i] / total.value)
                ? '-'
                : ((table.value.body[i] / total.value) * 100).toFixed(
...
function onSvgKeydown(event) {
    ...
    if (!noGhostDonut.value.length) return;
    ...
    if (isActivationKey) {
        ...
        const arc = noGhostDonut.value[activeTooltipIndex.value];
        if (!arc) return;
        selectDatapoint(arc, activeTooltipIndex.value);
        return;
    }
    ...
    const arc = noGhostDonut.value[nextIndex];
    if (!arc) return;
...
function setKeyboardTooltipPositionFromIndex(index) {
    ...
    let svgX = svg.value.width / 2;
    let svgY = svg.value.height / 2;

    if (FINAL_CONFIG.value.type === 'classic') {
        const arc = noGhostDonut.value[index];
        if (!arc?.arcSlice) return;

        const midpoint = findArcMidpoint(arc.arcSlice);
        if (!midpoint) return;

        svgX = midpoint.x;
        svgY = midpoint.y;
    } else {
        const polarMiddlePoint = polarAreas.value[index]?.middlePoint;
        if (!polarMiddlePoint) return;

        svgX = polarMiddlePoint.x;
        svgY = polarMiddlePoint.y;
    }
            ...
            >
                <g ref="G" class="vue-data-ui-g">
                        ...
                        <radialGradient
                            ...
                            :cx="
                                (isNaN((area.middlePoint.x / svg.width) * 100)
                                    ? 0
                                    : (area.middlePoint.x / svg.width) * 100) +
                                '%'
                            "
                            :cy="
                                (isNaN((area.middlePoint.y / svg.height) * 100)
                                    ? 0
                                    : (area.middlePoint.y / svg.height) * 100) +
                                '%'
                            "
                            r="62%"
                        >
                    ...
                    <template v-if="FINAL_CONFIG.type === 'classic'">
                        <g
                            v-for="(arc, i) in currentDonut.filter(
                                (el) => !el.ghost,
                            )"
                        >
                            <path
                        ...
                        >
                            <path
                                ...
                                :style="{
                                    transition:
                                        isFirstLoad ||
                                        !FINAL_CONFIG.serieToggleAnimation.show
                                            ? 'none'
                    ...
                    <g v-if="$slots.pattern">
                        <defs
                            v-for="(arc, i) in dataset"
                            :key="`pattern-${arc.patternIndex}`"
                        >
                            <slot
                                name="pattern"
                                v-bind="{
                            ...
                            >
                                <path
                                    ...
                                    :style="{
                                        transition:
                                            isFirstLoad ||
                                            !FINAL_CONFIG.serieToggleAnimation
                                                .show
                                                ? 'none'
                                                : `all ${FINAL_CONFIG.serieToggleAnimation.durationMs}ms ease-in-out`,
                                    }"
                    ...
                    <circle
                        ...
                        :cy="svg.height / 2"
                        :r="
                            /* This might require adjustments */ minSize <= 0
                                ? 10
                                : minSize
                        "
                    ...
                    <!-- DATALABELS -->
                    <g
                        v-for="(arc, i) in noGhostDonut.filter(
                            (el) => !el.ghost,
                        )"
                        :filter="getBlurFilter(i)"
                        ...
                        >
                            <foreignObject
                                :x="
                                    ...
                                    'end'
                                        ? calcMarkerOffsetX(arc).x - 120
                                        : calcMarkerOffsetX(arc, true)
                                                .anchor === 'middle'
                                          ? calcMarkerOffsetX(arc).x - 60
                                          : calcMarkerOffsetX(arc).x
                                "
                                :y="
                                    calcMarkerOffsetY(arc) - (isSafari ? 20 : 0)
                                "
                                width="120"
                                height="60"
                                    ...
                                    <slot
                                        ...
                                        v-bind="{
                                            ...
                                            isVisible:
                                                isArcBigEnough(arc) &&
                                                mutableConfig.dataLabels.show,
                                            textAlign: calcMarkerOffsetX(
                                                arc,
                                                true,
                                                16,
                                                true,
                                            ).anchor,
                                            flexAlign: calcMarkerOffsetX(
                                                arc,
                                                true,
                                                16,
                                            ).anchor,
                                            percentage: displayArcPercentage(
                                                arc,
                                                noGhostDonut,
                                            ),
                                        }"
                        ...
                        <g v-else>
                            <template v-if="FINAL_CONFIG.type === 'classic'">
                                <circle
                                    ...
                                    "
                                    :cx="
                                        smallArcLayoutsClassic[i]?.markerX ??
                                        calcMarkerOffsetX(arc).x
                                    "
                                    :cy="
                                        smallArcLayoutsClassic[i]?.markerY ??
                                ...
                                <text
                                    ...
                                    class="vue-data-ui-datalabel-inline"
                                    :text-anchor="
                                        smallArcLayoutsClassic[i]?.textAnchor ||
                                        calcMarkerOffsetX(arc, true, 12).anchor
                                    "
                                    :x="
                                        smallArcLayoutsClassic[i]?.labelX ??
                                        calcMarkerOffsetX(arc, true, 12).x
                                    "
                                    :y="
                            ...
                            <template v-if="FINAL_CONFIG.type === 'polar'">
                                <circle
                                    ...
                                    :cx="
                                        offsetFromCenterPoint({
                                            initX: polarAreas[i].middlePoint.x,
                                            initY: polarAreas[i].middlePoint.y,
                                            offset: 24,
                                            centerX: svg.width / 2,
                                        ...
                                        :x="
                                            offsetFromCenterPoint({
                                                initX: polarAreas[i].middlePoint
                                                    .x,
                                                initY: polarAreas[i].middlePoint
                                                    .y,
                                                offset: 42,
                                                centerX: svg.width / 2,
                                                centerY: svg.height / 2,
                            ...
                            >
                                <div>
                                    <slot
                                        ...
                                        :plot="{
                                            ...arc,
                                            textAlign: getPolarAnchor(
                                                polarAreas[i].middlePoint,
                                            ),
                                            flexAlign: getPolarAnchor(
                                                polarAreas[i].middlePoint,
                                            ),
                                            isFirstLoad,
                                        }"
