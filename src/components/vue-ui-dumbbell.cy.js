...
describe('<VueUiDumbbell />', () => {
    it('renders', () => {
        ...
        cy.mount(VueUiDumbbell, {
            ...
            cy.log('grid');
            cy.get('[data-cy="grid-line-y"]')
                .should('exist')
                .and('have.length', 9)
                .and('have.css', 'opacity', '1');
            cy.get('[data-cy="grid-line-x"]')
                .should('exist')
                .and('have.length', 6)
                .and('have.css', 'opacity', '1');
            cy.get('[data-cy="grid-base-x"]')
                .should('exist')
                .and('have.css', 'opacity', '1');

            cy.log('y labels');
            cy.get('[data-cy="label-y-name"]')
                .as('yLabelNames')
                .should('exist')
                .and('be.visible');
            ...
            cy.get('[data-cy="label-y-value"]')
                .should('exist')
                .and('be.visible')
                .and('contain', '%');

            cy.log('x labels');
            cy.get('[data-cy="label-x"]')
                .as('xLabels')
                .should('exist')
                .and('be.visible')
                .and('have.length', 9);
            ...
            cy.log('links');
            cy.get('[data-cy="link-curved"]')
                .should('exist')
                .and('be.visible')
                .and('have.length', dataset.length);

            cy.log('datapoints');
            cy.get('[data-cy="datapoint-start"]')
                .should('exist')
                .and('be.visible')
                .and('have.length', dataset.length);
            cy.get('[data-cy="datapoint-end"]')
                .should('exist')
                .and('be.visible')
                .and('have.length', dataset.length);
            cy.get('[data-cy="datapoint-label-start"]')
                .should('exist')
                .and('be.visible')
                .and('have.length', dataset.length);
            cy.get('[data-cy="datapoint-label-start"]')
                ...
                .and('be.visible')
                .and('have.length', dataset.length);
            cy.get('[data-cy="datapoint-label-end"]')
                .should('exist')
                .and('be.visible')
                .and('have.length', dataset.length);
            cy.get('[data-cy="datapoint-label-end"]')
