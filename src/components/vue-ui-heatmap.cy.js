...
describe('<VueUiHeatmap />', () => {
    function commonTest() {
        ...
        cy.log('cells');
        cy.get('[data-cy="cell-underlayer"]')
            .should('exist')
            .and('be.visible')
            .and('have.length', 91);
        cy.get('[data-cy="cell"]')
        ...
        cy.get('@xLabels').each((label, i) => {
            cy.wrap(label).contains(
                config.style.layout.dataLabels.xAxis.values[i],
            );
        });

        cy.log('legend');
        cy.get('[data-cy="legend-label-max"]')
            .should('exist')
            .and('be.visible')
            .and('contain', 30);
        cy.get('[data-cy="legend-label-min"]')
            .should('exist')
            .and('be.visible')
            .and('contain', 0);
