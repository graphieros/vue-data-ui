...
describe('<VueUiHistoryPlot />', () => {
    it('renders', () => {
        cy.mount(VueUiHistoryPlot, {
            ...
            cy.log('grid');
            cy.get('[data-cy="x-axis"]')
                .should('exist')
                .and('have.css', 'opacity', '1');
            cy.get('[data-cy="y-axis"]')
                .should('exist')
                .and('have.css', 'opacity', '1');
            cy.get('[data-cy="grid-line-vertical"]')
                .should('exist')
                .and('have.css', 'opacity', '1')
                .and('have.length', 8);
            cy.get('[data-cy="grid-line-horizontal"]')
                .should('exist')
                .and('have.css', 'opacity', '1')
                .and('have.length', 9);
            ...
            cy.log('axis names');
            cy.get('[data-cy="axis-name-y"]')
                .should('exist')
                .and('be.visible')
                .contains(config.style.chart.axes.y.name.text);
            cy.get('[data-cy="axis-name-x"]')
                .should('exist')
                .and('be.visible')
                .contains(config.style.chart.axes.x.name.text);

            cy.log('datapoints');
            cy.get('[data-cy="datapoint-path-wrapper"]')
                .should('exist')
                .and('be.visible')
                .and('have.length', dataset.length);
            cy.get('[data-cy="datapoint-path"]')
                .should('exist')
                .and('be.visible')
                .and('have.length', dataset.length);
            cy.get('[data-cy="datapoint-plot-underlayer"]')
                .should('exist')
                .and('be.visible')
                .and(
                    'have.length',
            ...
            cy.get('[data-cy="datapoint-index-label"]')
                .should('exist')
                .and('be.visible')
                .and(
                    'have.length',
