    ...
    it('renders', () => {
        cy.mount(VueUiHorizontalBar, {
            ...
            cy.get('[data-cy="datapoint-name"]')
                ...
                .and('be.visible')
                .and('have.length', 8);
            cy.get('[data-cy="datapoint-parent-name"]')
                .should('exist')
                .and('be.visible')
                .and('have.length', dataset.filter((d) => !!d.children).length);
            cy.get('[data-cy="datapoint-parent-value"]')
                .should('exist')
                .and('be.visible')
                .and('have.length', dataset.filter((d) => !!d.children).length);
        });
