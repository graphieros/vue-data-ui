...
const activeTrapIndex = ref(null); // a11y
...
const apexByKey = computed(() => {
    const map = {};
    radar.value.forEach((r) => {
        map[r.key] = { x: r.x, y: r.y };
    });
    return map;
});
...
const maxValue = computed(() => {
    return Math.max(
        ...Object.values(FINAL_DATASET.value).map((v) => (isNaN(v) ? 0 : v)),
    );
});

const grandTotal = computed(() => {
    return Object.values(FINAL_DATASET.value).reduce(
...
const convertedDataset = computed(() => {
    return Object.keys(FINAL_DATASET.value)
        .map((key, i) => {
            const value =
                typeof FINAL_DATASET.value[key] !== 'number' ||
                isNaN(FINAL_DATASET.value[key])
                    ? 0
                    : FINAL_DATASET.value[key];
            return {
                index: i,
                key,
                value,
...
const table = computed(() => {
    ...
    const body = convertedDataset.value.map((ds) =>
        isNaN(ds.value) ? 0 : ds.value,
    );
    return { head, body };
});
...
const dataTable = computed(() => {
    const head = [
        ...
        Number(
            grandTotal.value.toFixed(FINAL_CONFIG.value.table.td.roundingValue),
        ).toLocaleString(),
        '100%',
    ];
...
async function getImage({ scale = 2 } = {}) {
    ...
    const { imageUri, base64 } = await img({
        domElement: moodRadarChart.value,
        base64: true,
        img: true,
        scale,
    });
...
const orderedKeys = computed(() =>
    convertedDataset.value.map((datasetItem) => String(datasetItem.key)),
);
...
function getLegendItemByKey(key) {
    return convertedDataset.value.find(
        (datasetItem) => String(datasetItem.key) === String(key),
    );
}
...
function getTrapAriaLabel(key) {
    const datapoint = getLegendItemByKey(key);
    if (!datapoint) return `${key}`;

    return `${key}, ${getFormattedValue(datapoint.value, datapoint.index, datapoint)}, ${getFormattedPercentage(datapoint.value)}`;
}

function setActiveKeyByIndex(index) {
    const key = orderedKeys.value[index];
    if (!key) return;

    activeTrapIndex.value = index;
    selectedKey.value = key;
}
...
function onSvgBlur() {
    activeTrapIndex.value = null;
    selectedKey.value = null;
    isFocus.value = false;
}

function onSvgKeydown(event) {
    if (!svgRef.value || !orderedKeys.value.length || isAnnotator.value) return;
    ...
    if (isActivationKey) {
        if (activeTrapIndex.value === null) return;
        const key = orderedKeys.value[activeTrapIndex.value];
        if (!key) return;
        selectKey(key);
        return;
    }

    let nextIndex = activeTrapIndex.value;

    if (
        nextIndex === null ||
        nextIndex < 0 ||
        nextIndex >= orderedKeys.value.length
    ) {
        nextIndex = isNextKey ? 0 : orderedKeys.value.length - 1;
    } else if (isNextKey) {
        nextIndex += 1;
        if (nextIndex >= orderedKeys.value.length) {
            nextIndex = 0;
        }
    } else if (isPreviousKey) {
        nextIndex -= 1;
        if (nextIndex < 0) {
            nextIndex = orderedKeys.value.length - 1;
        }
    }

    setActiveKeyByIndex(nextIndex);
}

function onTrapKeydown(event, key) {
    const isActivationKey = event.key === 'Enter' || event.key === ' ';
    const isEscapeKey = event.key === 'Escape';
    if (!isActivationKey && !isEscapeKey) return;
    event.preventDefault();
    event.stopPropagation();
    if (isEscapeKey) {
        activeTrapIndex.value = null;
...
const a11yTable = computed(() => {
    ...
    const rows = convertedDataset.value.map((datasetItem) => [
        String(datasetItem.key),
        getFormattedValue(datasetItem.value, datasetItem.index, datasetItem),
        getFormattedPercentage(datasetItem.value),
    ]);
    return { headers, rows };
});
    ...
    >
        ...
        <div style="position: relative">
            ...
            >
                ...
                <line
                    data-cy="grid-radial"
                    v-for="line in outerPolygon.coordinates"
                    :x1="svg.width / 2"
                    :y1="svg.height / 2"
                    :x2="line.x"
                    :y2="line.y"
                ...
                <g
                    v-for="(plot, i) in radar.map((r) => r.plots)"
                    class="vue-ui-mood-radar-trap"
                    :style="`opacity:${selectedKey == plot.key ? '1' : '0'}`"
                >
                    <line
