...
const { dataset, config } = components.find(
    (c) => c.name === 'VueUiParallelCoordinatePlot',
);
        ...
        cy.mount(VueUiParallelCoordinatePlot, {
            ...
            cy.get('[data-cy="pcp-axis"]')
                ...
                .and(
                    ...
                    Math.max(
                        ...dataset.flatMap((d) =>
                            d.series.map((s) => s.values.length),
                        ),
                    ),
                );
            cy.get('[data-cy="pcp-axis-label"]')
                .as('axisLabels')
                .should('exist')
                .and('be.visible')
                .and(
            ...
            cy.log('scales');
            cy.get('[data-cy="scale-tick"]')
                .should('exist')
                .and('have.css', 'opacity', '1')
                .and('have.length', 37);
            cy.get('[data-cy="scale-label"]')
                .should('exist')
                .and('be.visible')
                .and('have.length', 37);
            ...
            cy.get('[data-cy="atom-shape"]')
                ...
                .and(
                    'have.length',
                    dataset
                        .flatMap((d) => d.series.map((s) => s.values.length))
                        .reduce((a, b) => a + b, 0) + 2 /* legend shapes */,
                );
            ...
            cy.get('[data-cy="datapoint-line"]')
                .should('exist')
                .and('be.visible')
                .and('have.length', dataset.flatMap((d) => d.series).length);
        });
    });
