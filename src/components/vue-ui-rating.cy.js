    ...
    it('renders with different config attributes', function () {
        cy.get('@fixture').then((fixture) => {
            ...
            function calculateAverageRating(source) {
                ...
                return Math.min(
                    fixture.config.to,
                    Math.max(fixture.config.from, averageRating),
                );
            }
            ...
            for (let i = 0; i < items; i += 1) {
                cy.get(`[data-cy="rating-shape-${i}"]`).should('exist');

                cy.get(`[data-cy="rating-active-trap-${i}"]`)
                    .should('exist')
                    .click();

                cy.get(`[data-cy="rating-position-bottom"]`).contains(
                    `${i + 1}`,
                );
            }
            ...
            cy.wait(100).then(() => {
                ...
                for (let i = 0; i < items; i += 1) {
                    cy.get(`[data-cy="rating-readonly-trap-${i}"]`).trigger(
                        'mouseenter',
                    );
