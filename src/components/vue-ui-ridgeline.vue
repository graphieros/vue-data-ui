...
const cfgAreas = computed(() => FINAL_CONFIG.value.style.chart.areas);
const cfgSelector = computed(() => FINAL_CONFIG.value.style.chart.selector);
const cfgXAxisLabels = computed(
    () => FINAL_CONFIG.value.style.chart.xAxis.labels,
);
            ...
            datapoints: [
                {
                    ...
                    color: '#999999',
                    values: [
                        28.639, 32.04, 41.134, 44.525, 21.151, 2.436, 0.218,
                        0.024, 0.002, 0, 0, 0,
                    ],
                },
                {
                    ...
                    color: '#CACACA',
                    values: [
                        13.253, 15.621, 23.36, 33.698, 29.935, 10.874, 2.364,
                        0.561, 0.107, 0.02, 0.006, 0.004,
                    ],
                },
            ],
        },
            ...
            datapoints: [
                {
                    ...
                    color: '#999999',
                    values: [
                        10.851, 13.195, 21.617, 36.556, 42.292, 21.006, 3.398,
                        0.223, 0.013, 0.001, 0, 0,
                    ],
                },
                {
                    name: '_',
                    color: '#CACACA',
                    values: [
                        3.171, 4.115, 8.108, 18.248, 31.641, 29.063, 12.031,
                        2.742, 0.504, 0.102, 0.032, 0.021,
                    ],
                },
            ],
        },
            ...
            datapoints: [
                {
                    ...
                    color: '#999999',
                    values: [
                        1.731, 2.334, 5.125, 13.626, 29.911, 38.524, 24.168,
                        7.646, 1.575, 0.317, 0.097, 0.063,
                    ],
                },
                {
                    name: '_',
                    color: '#CACACA',
                    values: [
                        0.25, 0.367, 1.026, 3.944, 13.635, 28.891, 30.149,
                        15.419, 4.714, 1.246, 0.442, 0.299,
                    ],
                },
            ],
        },
            ...
            datapoints: [
                {
                    ...
                    color: '#999999',
                    values: [
                        0.034, 0.054, 0.194, 1.065, 5.747, 20.735, 38.306,
                        32.899, 15.318, 5.566, 2.422, 1.76,
                    ],
                },
                {
                    name: '_',
                    color: '#CACACA',
                    values: [
                        0.001, 0.002, 0.009, 0.095, 1.124, 8.342, 27.115, 35.08,
                        21.449, 9.093, 4.243, 3.143,
                    ],
                },
            ],
        },
            ...
            datapoints: [
                {
                    ...
                    color: '#999999',
                    values: [
                        0, 0.001, 0.004, 0.051, 0.567, 3.322, 14.215, 44.783,
                        40.351, 20.377, 9.866, 7.378,
                    ],
                },
                {
                    name: '_',
                    color: '#CACACA',
                    values: [
                        0, 0, 0, 0, 0.001, 0.11, 4.136, 27.498, 43.24, 29.807,
                        17.345, 13.678,
                    ],
                },
            ],
            ...
            datapoints: [
                {
                    ...
                    color: '#999999',
                    values: [
                        0, 0, 0, 0, 0.025, 0.598, 3.886, 10.645, 54.479, 45.953,
                        30.814, 24.55,
                    ],
                },
                {
                    ...
                    color: '#CACACA',
                    values: [
                        0, 0, 0, 0, 0, 0, 0.007, 1.655, 26.63, 52.017, 45.192,
                        39.651,
                    ],
                },
            ],
...
const rowHeight = ref(
    Math.min(cfgAreas.value.height, cfgAreas.value.rowHeight),
);
...
function prepareChart() {
    ...
    rowHeight.value = Math.min(cfgAreas.value.height, cfgAreas.value.rowHeight);
    ...
    if (FINAL_CONFIG.value.responsive) {
        const handleResize = throttle(() => {
            const { width, height } = useResponsive({
                ...
                padding: cfgChart.value.padding,
            });
...
const { isPrinting, isImaging, generatePdf, generateImage } = usePrinter({
    ...
    fileName: cfgChart.value.title.text || 'vue-ui-ridgeline',
...
watch(
    ...
    () => {
        ...
        rowHeight.value = Math.min(
            cfgAreas.value.height,
            cfgAreas.value.rowHeight,
        );
        mutableConfig.value.showTable = FINAL_CONFIG.value.table.show;
        titleStep.value += 1;
        legendStep.value += 1;
...
const overlapRatio = computed(() => {
    return cfgAreas.value.height / cfgAreas.value.rowHeight;
});
...
const formattedDataset = computed(() => {
    ...
    return (FINAL_DATASET.value || []).map((ds) => ({
        ...ds,
        labelLen: measureSvgTextWidth(ds.name, cfgYAxisLabels.value.fontSize),
        uid: createUid(),
        datapoints: ds.datapoints.map((dp, j) => {
            const color = dp.color
                ? convertColorToHex(dp.color)
...
const baseHeight = computed(() => {
    return (
        ...
        rowHeight.value * (FINAL_DATASET.value || []).length +
        rowHeight.value * overlapRatio.value +
...
const maxDpLen = computed(() => {
    return Math.max(
        ...formattedDataset.value.flatMap((el) =>
            el.datapoints.map((dp) => dp.values.length),
        ),
    );
});
...
watchEffect(() => {
    ...
    (async () => {
        const labels = await useTimeLabels({
            ...
            start: 0,
            end: cfgXAxisLabels.value.values.length,
        });
...
const xAxisTrapsAndLabels = computed(() => {
    const maxLabelLen = Math.max(
        ...formattedDataset.value.map((el) => el.labelLen),
    );
    const startX =
        svg.value.padding.left +
        maxLabelLen +
        16 +
        cfgYAxisLabels.value.offsetX;
    ...
    for (let i = 0; i < maxDpLen.value; i += 1) {
        arr.push({
            ...
            label: cfgXAxisLabels.value.values[i]
...
const drawableDataset = computed(() => {
    ...
    const maxVal = Math.max(
        ...formattedDataset.value.flatMap((el) =>
            el.datapoints.flatMap((dp) => dp.values),
        ),
    );
    ...
    return formattedDataset.value.map((ds, i) => {
        ...
        return {
            ...ds,
            label: {
                x: startX - cfgYAxisLabels.value.fontSize,
                y:
                    (cfgYAxisLabels.value.centered ? zero - rh / 2 : zero) +
                    cfgYAxisLabels.value.offsetY,
            },
                ...
                .map((dp) => {
                    const plots = dp.values.map((v, k) => {
                        ...
                        const y =
                            ...
                            rowHeight.value *
                                overlapRatio.value *
                                (1 - ratioToMax(absoluteValue + absoluteMin));
...
function createXyDatasetForDialog(ds) {
    ...
    if (!cfgChart.value.dialog.show) return;
    ...
    xyConfig.value = deepEmptyObjectToNull({
        ...cfgChart.value.dialog.xyChart,
        responsive: true, // Overriding
        chart: {
            ...cfgChart.value.dialog.xyChart.chart,
            grid: {
                ...cfgChart.value.dialog.xyChart.chart.grid,
                labels: {
                    ...cfgChart.value.dialog.xyChart.chart.grid.labels,
                    xAxisLabels: {
                        ...cfgChart.value.dialog.xyChart.chart.grid.labels
                            .xAxisLabels,
                        values: cfgXAxisLabels.value.values, // Overriding
                        autoRotate: {
                            enable: true,
                            angle: cfgChart.value.dialog.xyChart.chart.grid
                                .labels.xAxisLabels.autoRotate.angle,
                        }, // overriding
                        datetimeFormatter:
                            cfgXAxisLabels.value.datetimeFormatter, // Overriding
                    },
                },
            },
            tooltip: {
                ...cfgChart.value.dialog.xyChart.chart.tooltip,
                showTimeLabel: cfgXAxisLabels.value.values.length > 0, // Overriding
            },
            userOptions: {
                ...cfgChart.value.dialog.xyChart.chart.userOptions,
                buttons: {
                    ...cfgChart.value.dialog.xyChart.chart.userOptions.buttons,
                ...
                useCursorPointer:
                    FINAL_CONFIG.value.userOptions.useCursorPointer, // Overriding
            },
        },
    });
...
function generateCsv(callback = null) {
    nextTick(() => {
        ...
        if (!callback) {
            downloadCsv({
                csvContent,
                title: cfgChart.value.title.text || 'vue-ui-ridgeline',
            });
        } else {
            callback(csvContent);
        }
...
async function getImage({ scale = 2 } = {}) {
    ...
    const { imageUri, base64 } = await img({
        domElement: ridgelineChart.value,
        base64: true,
        img: true,
        scale,
    });
...
useTimeLabelCollision({
    ...
    angle: cfgXAxisLabels.value.autoRotate.angle,
});
...
async function copyAlt(src = 'main-chart', ds = null, cfg = null) {
    ...
    await Promise.resolve(
        FINAL_CONFIG.value.userOptions.callbacks.altCopy({
            ...
            config: src === 'main-chart' ? FINAL_CONFIG.value : xyConfig.value,
            dataset:
                src === 'main-chart' ? drawableDataset.value : xyDataset.value,
        }),
    );
}
...
function onSvgFocus() {
    activeA11ySeriesIndex.value = null;
    isFocus.value = true;
}
...
function onSvgKeydown(event) {
    ...
    const isPreviousSeriesKey = event.key === 'ArrowUp';
    const isNextSeriesKey = event.key === 'ArrowDown';
    const isPreviousXKey = event.key === 'ArrowLeft';
    const isNextXKey = event.key === 'ArrowRight';
    ...
    if (isActivationKey) {
        if (activeA11ySeriesIndex.value !== null) {
            const ds = drawableDataset.value[activeA11ySeriesIndex.value];
            if (ds) {
                createXyDatasetForDialog(ds);
            }
            return;
        }

        if (selectedX.value) {
            selectX(selectedX.value);
        }
        return;
    }

    if (isPreviousSeriesKey || isNextSeriesKey) {
        let nextSeriesIndex = activeA11ySeriesIndex.value;

        if (nextSeriesIndex === null) {
            nextSeriesIndex = isNextSeriesKey
                ? 0
                : drawableDataset.value.length - 1;
        } else {
            nextSeriesIndex = getWrappedSeriesIndex(
                nextSeriesIndex + (isNextSeriesKey ? 1 : -1),
            );
        }

        activeA11ySeriesIndex.value = nextSeriesIndex;
        selectedYAxisLabelIndex.value = nextSeriesIndex;
        return;
    }

    if (!xAxisTrapsAndLabels.value.length) return;

    let nextXIndex = selectedX.value?.index ?? null;

    if (nextXIndex === null) {
        nextXIndex = isNextXKey ? 0 : xAxisTrapsAndLabels.value.length - 1;
    } else {
        nextXIndex = getWrappedXIndex(nextXIndex + (isNextXKey ? 1 : -1));
    }

    const trap = xAxisTrapsAndLabels.value[nextXIndex];
    if (!trap) return;

    onTrapEnter(trap);
}

function getWrappedSeriesIndex(index) {
    const len = drawableDataset.value.length;
    if (!len) return null;
    return ((index % len) + len) % len;
}

function getWrappedXIndex(index) {
    const len = xAxisTrapsAndLabels.value.length;
    if (!len) return null;
    return ((index % len) + len) % len;
}
...
<template>
    <div
        ref="ridgelineChart"
        :class="`vue-data-ui-component vue-ui-ridgeline ${isFullscreen ? 'vue-data-ui-wrapper-fullscreen' : ''}`"
    ...
    >
        ...
        <div style="position: relative">
            <svg
                ...
                :style="`max-width:100%;overflow:visible;background:transparent;color:${cfgChart.color};${FINAL_CONFIG.responsive ? `height: ${parentHeight}px; width: 100%;` : ''}`"
                tabindex="0"
                @focus="onSvgFocus"
                @blur="onSvgBlur"
            ...
            >
                ...
                <g v-for="(ds, i) in drawableDataset" :key="`ds-${i}`">
                    ...
                    <g v-for="(dp, j) in ds.datapoints" :key="dp.id">
                            ...
                            <defs>
                                <slot
                                    ...
                                    v-bind="{
                                        datapointIndex: i,
                                        seriesIndex: j,
                                        patternId: `pattern_${uid}_${dp.uid}`,
                                    }"
                                />
                            </defs>
                        </g>
                        ...
                        <path
                            fill="none"
                            :stroke="
                                cfgAreas.stroke.useSerieColor
                                    ? dp.color
                                    : cfgAreas.stroke.color
                            "
                            :stroke-width="cfgAreas.strokeWidth"
                            :d="
                                cfgAreas.smooth
                                    ? dp.smoothPathRidge
                                    : dp.straightPathRidge
                            "
                            stroke-linecap="round"
                        ...
                        <path
                            :fill="
                                !cfgAreas.useGradient
                                    ? dp.color
                                    : cfgAreas.useCommonColor
                                      ? `url(#gradient-${dp.id}-${uid})`
                                      : `url(#gradient-single-${uid}-${dp.uid})`
                            "
                            stroke="none"
                            :d="
                                cfgAreas.smooth
                        ...
                        <!-- ZERO LINE -->
                        <path
                            v-if="cfgChart.zeroLine.show"
                            :stroke="
                                cfgChart.zeroLine.useSerieColor
                                    ? dp.color
                                    : cfgChart.zeroLine.stroke
                            "
                            :stroke-dasharray="
                                cfgChart.zeroLine.strokeDasharray
                            "
                            :stroke-width="cfgChart.zeroLine.strokeWidth"
                            :d="dp.zeroPath"
                            stroke-linecap="round"
                        />
                        ...
                        <!-- MAX POINT INDICATOR -->
                        <template
                            v-if="cfgAreas.maxPoint.show && dp.plots.length > 1"
                        >
                                ...
                                <line
                                    ...
                                    :stroke="
                                        cfgAreas.maxPoint
                                            ...
                                            : cfgAreas.maxPoint.stroke
                                    "
                                    :stroke-width="
                                        cfgAreas.maxPoint.strokeWidth
                                    "
                                    stroke-linecap="round"
                                    :stroke-dasharray="
                                        cfgAreas.maxPoint.strokeDasharray
                                    "
                                />
                            </template>
                        </template>
                        ...
                        <template v-if="dp.plots.length === 1">
                            <circle
                                :cx="dp.plots[0].x"
                                :cy="dp.plots[0].y"
                                :stroke="cfgSelector.dot.stroke"
                                :stroke-width="cfgSelector.dot.strokeWidth"
                                :r="cfgSelector.dot.radius"
                                :fill="
                                    cfgSelector.dot.useDatapointColor
                                        ? dp.color
                                        : cfgSelector.dot.fill
                                "
                                :style="{
                                    pointerEvents: 'none',
                                }"
                    ...
                    <text
                        ...
                        :style="{
                            cursor:
                                cfgChart.dialog.show && isCursorPointer
                                    ? 'pointer'
                                    : 'default',
                        }"
                        :text-decoration="
                            cfgChart.dialog.show &&
                            (selectedYAxisLabelIndex === i ||
                                (!!selectedDatapoint &&
                                    ds.uid === selectedDatapoint.uid) ||
                                activeA11ySeriesIndex === i)
                                ? 'underline'
                                : ''
                        "
                ...
                <!-- X AXIS LABELS -->
                <g v-if="cfgXAxisLabels.values.length" ref="timeLabelsEls">
                    <template v-for="(xLabel, i) in xAxisTrapsAndLabels">
                        <slot
                            ...
                            v-bind="{
                                show:
                                    (xLabel &&
                                        !cfgXAxisLabels.showOnlyFirstAndLast &&
                                        !cfgXAxisLabels.showOnlyAtModulo) ||
                                    (xLabel &&
                                        cfgXAxisLabels.showOnlyFirstAndLast &&
                                        (i === 0 ||
                                            i ===
                                    ...
                                    (xLabel &&
                                        ...
                                        i %
                                            Math.max(
                                                ...
                                                Math.floor(
                                                    Number(
                                                        cfgChart.xAxis.labels
                                                            .modulo,
                                                    ) || 1,
                                                ),
                                            ) ===
                                            0),
                                fontSize: cfgXAxisLabels.fontSize,
                                content: xLabel.label,
                                textAnchor:
                                    cfgXAxisLabels.rotation > 0
                                        ? 'start'
                                        : cfgXAxisLabels.rotation < 0
                                          ? 'end'
                                          : 'middle',
                                fill: cfgXAxisLabels.color,
                                transform: `translate(${xLabel.selectorX}, ${drawingArea.top + xLabel.height + cfgXAxisLabels.offsetY}), rotate(${cfgXAxisLabels.rotation})`,
                                x: xLabel.selectorX,
                                y: drawingArea.bottom + cfgXAxisLabels.offsetY,
                            }"
                        >
                            <g
                                v-if="
                            ...
                            >
                                ...
                                <text
                                    ...
                                    :font-weight="
                                        cfgXAxisLabels.bold ? 'bold' : 'normal'
                                    "
                                    :transform="`translate(${xLabel.selectorX}, ${drawingArea.bottom + cfgXAxisLabels.offsetY}), rotate(${cfgXAxisLabels.rotation})`"
                                    :text-anchor="
                                        cfgXAxisLabels.rotation > 0
                                            ? 'start'
                                            : cfgChart.xAxis.labels.rotation < 0
                                              ? 'end'
                                              : 'middle'
                                    "
                                    :style="{
                                ...
                                <text
                                    ...
                                    v-html="
                                        createTSpansFromLineBreaksOnX({
                                            ...
                                            fontSize:
                                                cfgChart.xAxis.labels.fontSize,
                                            fill: cfgChart.xAxis.labels.color,
                                            stroke: cfgChart.backgroundColor,
                                            x: 0,
                                            y: 0,
                                        })
                ...
                <g>
                    ...
                    <!-- SELECTOR -->
                    <line
                        v-if="cfgSelector.show && !!selectedX"
                        :x1="selectedX.selectorX"
                        :x2="selectedX.selectorX"
                        :y1="selectedX.y"
                        :y2="selectedX.y + selectedX.height - rowHeight / 2"
                        :stroke="cfgSelector.stroke"
                        :stroke-width="cfgSelector.strokeWidth"
                        :stroke-dasharray="cfgSelector.strokeDasharray"
                        stroke-linecap="round"
                        :style="{ pointerEvents: 'none' }"
                    />

                    <!-- SELECTED X -->
                    <template v-if="!!selectedX || cfgSelector.labels.showLast">
                            ...
                            <template v-for="dp in ds.datapoints">
                                <template v-for="(plot, k) in dp.plots">
                                    ...
                                    <circle
                                        ...
                                        :stroke="cfgSelector.dot.stroke"
                                        :stroke-width="
                                            cfgSelector.dot.strokeWidth
                                        "
                                        :r="cfgSelector.dot.radius"
                                        :fill="
                                            cfgSelector.dot.useDatapointColor
                                                ? dp.color
                                                : cfgChart.selector.dot.fill
                                        "
                                        :style="{
                                            pointerEvents: 'none',
                                        }"
                                    ...
                                    <text
                                        ...
                                        :text-anchor="
                                            isTextOverflowingRight(
                                                ...
                                                : 'start'
                                        "
                                        :font-size="cfgSelector.labels.fontSize"
                                        :fill="cfgSelector.labels.color"
                                        :style="{
                                            pointerEvents: 'none',
                                        }"
                                    >
                                        {{
                                            applyDataLabel(
                                                ...
                                                plot.value,
                                                dataLabel({
                                                    p: cfgChart.xAxis.labels
                                                        .prefix,
                                                    v: plot.value,
                                                    s: cfgChart.xAxis.labels
                                                        .suffix,
                                                    r: cfgChart.selector.labels
                                                        .rounding,
                                                }),
                                            )
                                        }}
                    ...
                    <template #td="{ td }">
                        {{
                            td.name
                                ? td.name
                                : applyDataLabel(
                                      cfgSelector.labels.formatter,
                                      td,
                                      dataLabel({
                                          p: cfgChart.xAxis.labels.prefix,
                                          v: td,
                                          s: cfgChart.xAxis.labels.suffix,
                                          r: FINAL_CONFIG.table.td
                                              .roundingValue,
                                      }),
                                  )
                        }}
        ...
        <BaseDraggableDialog
            v-if="cfgChart.dialog.show"
            ref="dialog"
            @close="selectedDatapoint = null"
            :backgroundColor="cfgChart.dialog.backgroundColor"
            :color="cfgChart.dialog.color"
            :headerBg="cfgChart.dialog.header.backgroundColor"
            :headerColor="cfgChart.dialog.header.color"
            :isFullscreen="isFullscreen"
