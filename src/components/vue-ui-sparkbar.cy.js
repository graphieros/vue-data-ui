...
describe('<VueUiSparkbar />', () => {
        ...
        cy.mount(VueUiSparkbar, {
            ...
            for (let i = 0; i < dataset.length; i += 1) {
                ...
                if (config.style.layout.showTargetValue) {
                    ...
                    const target =
                        config.style.layout.target ?? dataset[i].target ?? 0;
                    ...
                    const expectedText =
                        `${targetValueText} ${formattedValue}`.trim();
    ...
    it('renders with custom title and subtitle using slots', () => {
        ...
        cy.mount(VueUiSparkbar, {
            ...
            cy.get('[data-cy="custom-subtitle"]')
                .should('exist')
                .contains(customSubtitle);
    ...
    it('renders with default title when no slot is provided', () => {
        cy.mount(VueUiSparkbar, {
            ...
            cy.get('[data-cy="sparkbar-title"]')
                .should('exist')
                .contains(config.style.title.text);
            cy.get('[data-cy="sparkbar-subtitle"]')
                .should('exist')
                .contains(config.style.title.subtitle.text);
        });
    });
    ...
    it('emits', () => {
        cy.mount(VueUiSparkbar, {
            ...
            cy.get('[data-cy="datapoint-wrapper"]')
                .first()
                .click({ force: true })
                .then(() => {
                    expect(
