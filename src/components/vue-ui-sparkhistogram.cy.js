    ...
    it('renders', () => {
        cy.mount(VueUiSparkhistogram, {
            ...
            cy.get('[data-cy="subtitle"]')
                .should('exist')
                .and('be.visible')
                .and('contain', config.style.title.subtitle.text);
            ...
            cy.get('[data-cy="title-selection-value-label"]')
                .should('exist')
                .and('be.visible')
                .and('contain', dataset[0].valueLabel);
            ...
            cy.get('[data-cy="datapoint-label-value"]')
                .as('valueLabels')
                .should('exist')
                .and('be.visible')
                .and('have.length', dataset.length);
            cy.get('@valueLabels').each((label, i) => {
                cy.wrap(label).contains(
                    dataLabel({
                        v: dataset[i].value,
                        r: config.style.labels.value.rounding,
                    }),
                );
            });
            cy.get('[data-cy="datapoint-label-valueLabel"]')
                .as('valueLabels2')
                .should('exist')
                .and('be.visible')
                .and('have.length', dataset.length);
            ...
            cy.get('[data-cy="datapoint-label-time"]')
                .as('timeLabels')
                .should('exist')
                .and('be.visible')
                .and('have.length', dataset.length);
