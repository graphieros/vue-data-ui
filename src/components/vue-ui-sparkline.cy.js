...
function commonTest(line = true) {
    ...
    cy.get('[data-cy="title"]')
        .should('exist')
        .and('be.visible')
        .and('contain', config.style.title.text);

    cy.log('data label');
    cy.get('[data-cy="sparkline-datalabel"]')
        .should('exist')
        .and('be.visible')
        .and('contain', dataset.at(-1).value);

    cy.log('selection');
    cy.get('[data-cy="tooltip-trap"]')
        .first()
        .trigger('mouseenter', { force: true });
    cy.get('[data-cy="selection-indicator"]')
        .should('exist')
        .and('have.css', 'opacity', '1');
    cy.get('[data-cy="sparkline-datalabel"]')
        .should('exist')
        .and('be.visible')
        .and('contain', dataset[0].value);
...
describe('<VueUiSparkline />', () => {
    it('renders (line)', () => {
        cy.mount(VueUiSparkline, {
            ...
            cy.log('area');
            cy.get('[data-cy="sparkline-angle-area"]')
                .should('exist')
                .and('be.visible');

            cy.log('line');
            cy.get('[data-cy="sparkline-straight-line"]')
                .should('exist')
                .and('be.visible');
        });
    });
    ...
    it('renders (spline)', () => {
        cy.mount(VueUiSparkline, {
            ...
            cy.log('area');
            cy.get('[data-cy="sparkline-smooth-area"]')
                .should('exist')
                .and('be.visible');

            cy.log('line');
            cy.get('[data-cy="sparkline-smooth-path"]')
                .should('exist')
                .and('be.visible');
        });
    });
    ...
    it('emits', () => {
        cy.mount(VueUiSparkline, {
            ...
            cy.get('[data-cy="tooltip-trap"]')
                ...
                .trigger('mouseenter', { force: true })
                .then(() => {
                    expect(wrapper.emitted('hoverIndex')[0][0]).to.have.keys(
                        'index',
                    );
                    expect(wrapper.emitted('hoverIndex')[0][0].index).to.equal(
                        0,
                    );
                })
                .then(() => {
                    cy.get('[data-cy="tooltip-trap"]')
                        ...
                        .then(() => {
                            expect(
                                wrapper.emitted('hoverIndex')[1][0],
                            ).to.have.keys('index');
                            expect(
                                wrapper.emitted('hoverIndex')[1][0].index,
                            ).to.equal(undefined);
                        });
                });
