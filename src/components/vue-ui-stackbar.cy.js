...
describe('<VueUiStackbar />', () => {
    function commonTest() {
        ...
        cy.get('[data-cy="axis-label-x"]')
            .should('exist')
            .and('be.visible')
            .and('contain', config.style.chart.grid.x.axisName.text);
        cy.get('[data-cy="axis-label-y"]')
            .should('exist')
            .and('be.visible')
            .and('contain', config.style.chart.grid.y.axisName.text);

        cy.log('scale labels');
        cy.get('[data-cy="scale-line-y"]')
            .should('exist')
            .and('have.length', 9);
        cy.get('[data-cy="scale-label-y"]')
            .as('yLabels')
            .should('exist')
            .and('be.visible')
            .and('have.length', 9);
        ...
        cy.get('[data-cy="time-label"]')
            ...
            .and(
                'have.length',
                Math.max(...dataset.map((d) => d.series.length)),
            );
