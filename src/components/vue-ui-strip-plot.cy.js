...
describe('<VueUiStripPlot />', () => {
    it('renders', () => {
        ...
        cy.mount(VueUiStripPlot, {
            ...
            cy.log('grid');
            cy.get('[data-cy="grid-horizontal"]')
                .should('exist')
                .and('have.css', 'opacity', '1')
                .and('have.length', 6);
            cy.get('[data-cy="grid-vertical"]')
                .should('exist')
                .and('have.css', 'opacity', '1')
            ...
            cy.get('[data-cy="axis-y-name"]')
                .should('exist')
                .and('be.visible')
                .and('contain', config.style.chart.labels.axis.yLabel);
            cy.get('[data-cy="axis-x-name"]')
                .should('exist')
                .and('be.visible')
                .and('contain', config.style.chart.labels.axis.xLabel);

            cy.log('datapoint selection');
            cy.get('[data-cy="atom-shape"]')
                .eq(2)
                .trigger('mouseenter', { force: true });
            cy.get('[data-cy="selection-line"]')
                .should('exist')
                .and('have.css', 'opacity', '1');
            cy.get('[data-cy="selection-marker-circle-left"]')
                .should('exist')
                .and('be.visible');
            cy.get('[data-cy="selection-marker-circle-right"]')
                .should('exist')
                .and('be.visible');
            ...
            cy.get('[data-cy="atom-shape"]')
                ...
                .and(
                    'have.length',
                    dataset
                        .map((d) => d.plots.length)
                        .reduce((a, b) => a + b, 0) * 2,
                );
            cy.get('[data-cy="best-plot-label"]')
                .as('bestPlotLabels')
                .should('exist')
                .and('have.length', dataset.length);
            cy.get('@bestPlotLabels').each((label, i) => {
                cy.wrap(label).contains(
                    dataset[i].plots.reduce(
