...
describe('<VueUiTableHeatmap />', () => {
    ...
    it('renders with slots', () => {
        cy.mount(VueUiTableHeatmap, {
            ...
            slots: {
                head: ({ value }) =>
                    h('div', { 'data-cy': 'slot-head' }, value),
                rowTitle: ({ value }) =>
                    h('div', { 'data-cy': 'slot-row-title' }, value),
                ...
                average: ({ value }) =>
                    h('div', { 'data-cy': 'slot-average' }, value.toFixed(0)),
                median: ({ value }) =>
                    h('div', { 'data-cy': 'slot-median' }, value.toFixed(0)),
            },
        }).then(() => {
            testCommonFeatures({
                userOptions: true,
            ...
            cy.log('head slot');
            cy.get('[data-cy="slot-head"]')
                .as('head')
                .should('be.visible')
                .and('have.length', config.table.head.values.length);
            cy.get('@head').each((th, i) => {
                cy.wrap(th).contains(config.table.head.values[i]);
            });

            cy.log('rowTitle slot');
            cy.get('[data-cy="slot-row-title"]')
                .as('rowTitle')
                .should('exist')
                .and('be.visible');
            ...
            cy.log('cell slot');
            cy.get('[data-cy="slot-cell"]')
                .should('exist')
                .and('be.visible')
                .and('have.length', dataset.length * dataset[0].values.length);

            cy.log('sum slot');
            cy.get('[data-cy="slot-sum"]')
                .as('sum')
                .should('exist')
                .and('be.visible')
            ...
            cy.get('@avg').each((avg, i) => {
                const average =
                    dataset[i].values.reduce((a, b) => a + b, 0) /
                    dataset[i].values.length;
                cy.wrap(avg).contains(average.toFixed(0));
            });

            cy.log('median slot');
            cy.get('[data-cy="slot-median"]')
                .as('med')
                .should('exist')
                .and('be.visible')
                .and('have.length', dataset.length);
