...
function calcTickStart(angle, distance = 1) {
    ...
    return {
        x:
            wheel.value.centerX +
            wheel.value.radius *
                Math.cos(angleStart + (angle * Math.PI) / 180) *
                distance,
        y:
            wheel.value.centerY +
            wheel.value.radius *
                Math.sin(angleStart + (angle * Math.PI) / 180) *
                distance,
    };
}
...
function buildTicks3D({
    ...
    for (let i = 0; i < count; i += 1) {
        ...
        const depth =
            (Math.max(rzo, rzi) - -outerR * Math.sin(ax)) /
        ...
        const stroke = Math.max(
            ...
            baseStroke *
                so *
                (Math.min(svg.value.width, svg.value.height) / 360),
        );
...
<template>
    <div
        ...
        :class="{
            ...
            'vue-ui-wheel-3d-wrap': FINAL_CONFIG.layout === '3d',
        }"
        ref="wheelChart"
        :id="uid"
    ...
    >
        ...
        <svg
            ...
            :class="{
                ...
                'vue-ui-wheel-3d-svg': FINAL_CONFIG.layout === '3d',
            }"
            data-cy="wheel-svg"
            :viewBox="
                FINAL_CONFIG.layout === '3d' && !FINAL_CONFIG.responsive
        ...
        >
            ...
            <foreignObject
                ...
                :width="
                    FINAL_CONFIG.layout === '3d' && !FINAL_CONFIG.responsive
                        ? (vb3D?.w + 20 ?? Math.max(10, svg.width))
                        : Math.max(10, svg.width)
                "
                :height="
                    FINAL_CONFIG.layout === '3d' && !FINAL_CONFIG.responsive
                        ? (vb3D?.h ?? Math.max(10, svg.height))
                        : Math.max(10, svg.height)
                "
                :style="{
                    pointerEvents: 'none',
                }"
            ...
            >
                ...
                <text
                    ...
                    :class="{
                        'vue-ui-wheel-label': FINAL_CONFIG.layout === '3d',
                    }"
                >
                    {{ percentageText }}
                </text>
