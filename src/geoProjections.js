...
const projections = {
    ...
    robinson([lon, lat], width, height, center) {
        ...
        const [A1, B1] =
            robinsonTable[Math.min(index + 1, robinsonTable.length - 1)];
    ...
    mollweide([lon, lat], width, height, center) {
        ...
        while (Math.abs(delta) > epsilon && i < 10) {
            delta =
                -(2 * theta + Math.sin(2 * theta) - Math.PI * Math.sin(φ)) /
    ...
    winkelTripel([lon, lat], width, height, center) {
        ...
        const x2 =
            (2 * Math.cos(φ) * Math.sin(λ / 2)) /
            (Math.sin(alpha) / alpha || 1);
    ...
    hammer([lon, lat], width, height, center) {
        ...
        const x =
            width / 2 +
            ((width / 2) *
                ((2 * Math.SQRT2 * Math.cos(φ) * Math.sin(λ / 2)) / denom)) /
                2.8284271247461903;
        const y =
            height / 2 -
            ((height / 2) * ((Math.SQRT2 * Math.sin(φ)) / denom)) /
                1.4142135623730951;
        return [x, y];
    },
    ...
    gallPeters([lon, lat], width, height, center) {
        ...
        const x = ((lon - lon0 + 180) / 360) * width;
        const y =
            height / 2 - (height / Math.PI) * Math.sin((lat * Math.PI) / 180);
        return [x, y];
    },
        ...
        if (absφ < epsilon) {
            ...
            y =
                (Math.sign(φ) *
                    pi *
                    (P * Q -
                        A * Math.sqrt(Math.max(0, (A2 + 1) * P2_A2 - Q * Q)))) /
                P2_A2;
        }
    ...
    azimuthalEquidistant([lon, lat], width, height, center = [0, 0]) {
        ...
        const cx = width / 2,
            cy = height / 2;
        return [cx + xProj * scale, cy - yProj * scale];
    },
};
...
function mergeCountries(features, parent, child) {
    ...
    if (parentFeature && childFeature) {
        ...
        if (childFeature.geometry.type === 'Polygon') {
            parentFeature.geometry.coordinates.push(
                childFeature.geometry.coordinates,
            );
        ...
        features = features.filter(
            (f) =>
                !(f.properties.admin === child || f.properties.name === child),
        );
    }
    return features;
}
