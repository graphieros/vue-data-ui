...
const { isPrinting, isImaging, generatePdf, generateImage } = usePrinter({
    ...
    fileName:
        FINAL_CONFIG.value.style.chart.title.text || 'vue-ui-vertical-bar',
...
async function autoSize() {
    ...
    const parentLabels = G.value.querySelectorAll(
        '.vue-ui-horizontal-bar-parent-label',
    );
    const childrenLabels = G.value.querySelectorAll(
        '.vue-ui-horizontal-bar-child-label',
    );

    if (!!parentLabels && parentLabels.length) {
        const maxW = Math.max(
            ...Array.from(parentLabels).flatMap((pl) => {
                const texts = pl.querySelectorAll('text');
                if (!texts.length) return 0;
                return Array.from(texts).map((t) => t.getBBox().width);
            }),
        );
        const maxC = Math.max(
            ...Array.from(childrenLabels).flatMap((pl) => {
                const texts = pl.querySelectorAll('text');
                if (!texts.length) return 0;
                return Array.from(texts).map((t) => t.getBBox().width);
            }),
        ...
        Array.from(parentLabels).forEach((pl) => {
            const texts = pl.querySelectorAll('text');
...
function prepareChart() {
    ...
    barCount.value = FINAL_DATASET.value
        .flatMap((serie) => {
    ...
    if (FINAL_CONFIG.value.responsive) {
        ...
        FINAL_CONFIG.value.autoSize && autoSize();
    }
}
...
const svg = computed(() => {
    return {
        ...
        padding: {
            top: 12,
            left:
                (FINAL_CONFIG.value.autoSize ? 0 : 128) +
                FINAL_CONFIG.value.style.chart.layout.bars.offsetX,
            right:
                (FINAL_CONFIG.value.autoSize ? 0 : 64) +
                FINAL_CONFIG.value.style.chart.layout.bars.paddingRight,
            bottom: 12,
        },
    };
});
...
async function recalculateHeight() {
    barCount.value = mutableDataset.value
        .flatMap((serie) => {
            if (serie.children && serie.children.length > 0) {
                return serie.children.length;
            } else {
...
function getParentData(serie, index) {
    ...
    const start =
        drawableArea.value.top + (barGap.value + barHeight.value) * index;
    ...
    return {
        ...
        percentageToTotal: isNaN(parent.value / total.value)
            ...
            : calcProportionToTotal(
                  parent.value,
                  true,
                  FINAL_CONFIG.value.style.chart.layout.bars.dataLabels
                      .percentage.roundingPercentage,
              ),
        sign: parent.sign,
    };
}
    ...
    if (
        ...
        if (FINAL_CONFIG.value.style.chart.tooltip.showValue) {
            html += `<div>${FINAL_CONFIG.value.translations.value}: <b>${applyDataLabel(
                FINAL_CONFIG.value.style.chart.layout.bars.dataLabels.value
                    .formatter,
...
function makeDataLabel(value, datapoint, seriesIndex, sign) {
    ...
    const label = applyDataLabel(
        ...
        dataLabel({
            p: FINAL_CONFIG.value.style.chart.layout.bars.dataLabels.value
                .prefix,
            v: checkNaN(sign === -1 ? (value >= 0 ? -value : value) : value),
            s: FINAL_CONFIG.value.style.chart.layout.bars.dataLabels.value
                .suffix,
...
<template>
    <div
        :class="`vue-data-ui-component vue-ui-vertical-bar ${isFullscreen ? 'vue-data-ui-wrapper-fullscreen' : ''} ${FINAL_CONFIG.useCssAnimation ? '' : 'vue-ui-dna'} ${FINAL_CONFIG.responsive && FINAL_CONFIG.autoSize ? 'vue-ui-vertical-bar-autosize' : ''}`"
        ref="verticalBarChart"
        :id="`vue-ui-vertical-bar_${uid}`"
        :style="`padding-top:${FINAL_CONFIG.responsive && FINAL_CONFIG.autoSize ? `${remainingHeight / 2}px` : '0'}; font-family:${FINAL_CONFIG.style.fontFamily};width:100%; text-align:center;background:${FINAL_CONFIG.style.chart.backgroundColor};${FINAL_CONFIG.responsive ? (FINAL_CONFIG.autoSize ? `height:calc(100% - ${remainingHeight}px` : 'height: 100%') : ''}`"
            ...
            <g ref="G" class="vue-data-ui-g">
                ...
                <g v-for="(serie, i) in bars">
                    ...
                    <rect
                        ...
                        :fill="
                            FINAL_CONFIG.style.chart.layout.bars.underlayerColor
                        "
                ...
                <g v-for="(serie, i) in bars">
                    ...
                    <rect
                        ...
                        :height="barHeight <= 0 ? 0.0001 : barHeight"
                        :fill="
                            FINAL_CONFIG.style.chart.layout.bars.useGradient
                        ...
                        :rx="FINAL_CONFIG.style.chart.layout.bars.borderRadius"
                        :stroke="
                            FINAL_CONFIG.style.chart.layout.bars.useStroke
                                ? serie.color
                                : 'none'
                        "
                        :stroke-width="
                    ...
                    <line
                        ...
                        :x1="viewBoxParts.x"
                        :x2="
                            FINAL_CONFIG.style.chart.layout.separators.fullWidth
                                ? viewBoxParts.width - Math.abs(viewBoxParts.x)
                                : drawableArea.left
                        "
                        :y1="
                            barHeight +
                    ...
                    <!-- DATALABELS -->
                    <g class="vue-ui-horizontal-bar-child-label">
                        <text
                        ...
                        <text
                            ...
                            v-if="
                                ...
                                FINAL_CONFIG.style.chart.layout.bars.nameLabels
                                    .show
                            "
                            text-anchor="end"
                            :x="
                    ...
                    <!-- PARENT NAMES -->
                    <g class="vue-ui-horizontal-bar-parent-label">
                        <text
                        ...
                        >
                            {{
                                ...
                                getParentData(serie, i),
                                i,
                                serie.parentSign || serie.sign)
                            }}
                        </text>
                    </g>
                ...
                <slot name="svg" :svg="viewBoxParts" />
            </g>
        </svg>
                                            ...
                                            >
                                                {{
                                                    [
                                                        ...
                                                        ? ''
                                                        : `${(tr.percentageToTotal * 100).toFixed(FINAL_CONFIG.table.td.roundingPercentage)}%`
                                                }}
                                            </div>
                                        </td>
                                        <td
                                            ...
                                            >
                                                {{
                                                    [
                                                        ...
                                                        ? ''
                                                        : `${(tr.childPercentageToParent * 100).toFixed(FINAL_CONFIG.table.td.roundingPercentage)}%`
                                                }}
                                            </div>
                                        </td>
                                        <td
                                            ...
                                            >
                                                {{
                                                    [
                                                        ...
                                                        ? ''
                                                        : `${(tr.childPercentageToTotal * 100).toFixed(FINAL_CONFIG.table.td.roundingPercentage)}%`
                                                }}
                                            </div>
                                        </td>
                                    </tr>
