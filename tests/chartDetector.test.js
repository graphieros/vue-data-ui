...
describe('detectChart', () => {
    ...
    test('warns of insufficient dataset when just a number is provided', () => {
        ...
        expect(consoleMock).toHaveBeenCalledWith(
            'The provided dataset (1) is not sufficient to build a chart',
        );
    });
    ...
    test('warns of insufficient dataset when just a string is provided', () => {
        ...
        expect(consoleMock).toHaveBeenCalledOnce();
        expect(consoleMock).toHaveBeenCalledWith(
            'The provided dataset (z) is not sufficient to build a chart',
        );
    });

    test('returns a dataset and a recommended chart type of bar for a set containing an array of numbers', () => {
        expect(detectChart({ dataset: datasetNumbers(5) }).type).toBe(
            chartType.BAR,
        );
        expect(
            detectChart({ dataset: datasetNumbers(12), barLineSwitch: 20 })
                .type,
        ).toBe(chartType.BAR);
    });

    test('returns a dataset and a recommended chart type of line for a set containing an array of numbers', () => {
        expect(detectChart({ dataset: datasetNumbers(12) }).type).toBe(
            chartType.LINE,
        );
    });

    test('returns a dataset and a recommend chart type of donut', () => {
        expect(detectChart({ dataset: datasetObjectDonut(3) }).type).toBe(
            chartType.DONUT,
        );
    });

    test('returns a dataset and a recommend chart type of line', () => {
        expect(detectChart({ dataset: datasetObjectXy(1, 7) }).type).toBe(
            chartType.LINE,
        );
    });

    test('returns a dataset and a recommend chart type of bar', () => {
        expect(detectChart({ dataset: datasetObjectXy(1, 3) }).type).toBe(
            chartType.BAR,
        );
    });
});
...
describe('isSimpleArrayOfStrings', () => {
    test('returns true if passed an array of strings', () => {
        ...
        expect(isSimpleArrayOfStrings([{ a: 'X', b: 'Y', c: 'Z' }])).toBe(
            false,
        );
...
describe('passesDatatypeCheck', () => {
    test('returns true if any value of a dataset object matches a given type', () => {
        ...
        expect(
            passesDatatypeCheck(Object.values(datasetObject1), (v) =>
                Array.isArray(v),
            ),
        ).toBe(true);
        expect(
...
describe('getFirstEntryMatch', () => {
    test('returns the first entry in an object that matches a condition', () => {
        ...
        expect(
            getFirstEntryMatch(dataset1[0], (v) => typeof v === 'string'),
        ).toBe('Serie name 0');
        expect(
            getFirstEntryMatch(dataset1[0], (v) => isSimpleArrayOfNumbers(v)),
        ).toStrictEqual([0, 1, 2]);
    });
});
