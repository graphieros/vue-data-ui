...
describe('useDateTime composable', () => {
    const dtLocal = useDateTime({
        useUTC: false,
        min: undefined,
        max: undefined,
        locale,
    });
    const dtUTC = useDateTime({
        useUTC: true,
        min: undefined,
        max: undefined,
        locale,
    ...
    test('formatDate produces correct patterns in UTC mode', () => {
        ...
        // Standard tokens
        expect(dtUTC.formatDate(date, 'yyyy-MM-dd HH:mm:ss fff')).toBe(
            '2022-01-02 03:04:05 006',
        );
        expect(dtUTC.formatDate(date, 'ddd, MMM dd, yyyy')).toBe(
            'Sun, Jan 02, 2022',
        );
        ...
        expect(dtUTC.formatDate(date, 'yyyy-MM-dd\\THH:mm:ss.fffK')).toBe(
